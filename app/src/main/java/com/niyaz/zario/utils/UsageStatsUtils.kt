...
/**
 * Screen-time helper that mirrors the Java `TodayUsageCalculator` logic.
 * All calculations now rely exclusively on UsageEvents pairing and explicit
 * event stitching so that behaviour stays deterministic across the app.
 */
object UsageStatsUtils {
    ...
    fun getCurrentDayScreenTimePrecise(context: Context): Long {
        return computeTotalsForWindow(context, startOfTodayMillis(), System.currentTimeMillis()).values.sum()
    }

    fun computeTodayTotals(context: Context): Map<String, Long> {
        return computeTotalsForWindow(context, startOfTodayMillis(), System.currentTimeMillis())
    }
    ...
    fun getGlobalUsageSummary(
        ...
        repeat(requestedDays) { index ->
            ...
            val totals = computeTotalsForWindow(context, startMs, endMs)
