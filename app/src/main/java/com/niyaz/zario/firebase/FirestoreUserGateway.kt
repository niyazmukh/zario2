...
class FirestoreUserGateway @Inject constructor(
    ...
    suspend fun recordCycleResult(
        ...
        firestore.runBatch { batch ->
            ...
            batch.set(historyDoc, historyData, SetOptions.merge())
        }.await()
    ...
    suspend fun fetchUserState(userId: String): RemoteUserState {
        ...
        val planMap = snapshot.get(FirestoreFields.PLAN) as? Map<String, Any?>
        val plan = planMap?.toScreenTimePlan()
    ...
    suspend fun fetchEvaluationCycles(
        ...
        val historySnapshots = query.get().await().documents
        ...
        return historySnapshots.map { historyDoc ->
            ...
            val hourlySnapshots = historyDoc.reference
                .collection(FirestoreCollections.HOURLY_USAGE)
                .get()
                .await()
                .documents
