...
/* loaded from: C:\Users\niyaz\Downloads\universal\smali\com\google\android\gms\tasks\Tasks.smali */
public final class Tasks {
    ...
    public static <TResult> TResult await(Task<TResult> task) throws InterruptedException {
        ...
        C.g(task, "Task must not be null");
        if (task.isComplete()) {
    ...
    public static <T> Task<T> withTimeout(Task<T> task, long j5, TimeUnit timeUnit) {
        C.g(task, "Task must not be null");
        C.a("Timeout must be positive", j5 > 0);
        C.g(timeUnit, "TimeUnit must not be null");
    ...
    public static <TResult> Task<TResult> call(Executor executor, Callable<TResult> callable) {
        C.g(executor, "Executor must not be null");
        C.g(callable, "Callback must not be null");
        zzw zzwVar = new zzw();
    ...
    public static <TResult> TResult await(Task<TResult> task, long j5, TimeUnit timeUnit) throws TimeoutException {
        if (Looper.getMainLooper() != Looper.myLooper()) {
            Looper looperMyLooper = Looper.myLooper();
            if (looperMyLooper != null && Objects.equals(looperMyLooper.getThread().getName(), "GoogleApiHandler")) {
                throw new IllegalStateException("Must not be called on GoogleApiHandler thread.");
